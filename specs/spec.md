...
- **Stack**: NestJS + TypeScript on the backend, structured with Domain-Driven Design layering per bounded context (domain/application/infrastructure/presentation), React + TypeScript (Vite, no Next.js) on the front-end, and a standalone lightweight Express service simulating the ERP as a real external HTTP dependency — per the project [constitution](constitution.md).
...
- **Backend, unit** (Jest, `*.spec.ts`, mocked/faked collaborators — required by the project [constitution](constitution.md) on top of the e2e seam above): the two places that hold actual business rules — `StockService`, the Inventory domain service (reservation only succeeds while there's enough available; release restores availability without touching base stock; confirmation permanently deducts it) and `CheckoutUseCase`, the Checkout application service (each error branch throws the right domain error; a cached idempotency key short-circuits before creating a new order). The repository implementations (`InMemoryProductRepository`, `InMemoryOrderRepository`, etc.), `IdempotencyService`, and `HttpErpGateway` are plain data holders/adapters with no branching logic of their own — they're exercised through the e2e tests and don't get redundant isolated unit tests of their own.
