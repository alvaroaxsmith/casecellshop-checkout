...
> **Amendment (superseded architecture):** This plan describes the original Domain-Driven Design task-by-task design (entities, repository ports/adapters, application-layer use cases, per-bounded-context `domain/application/infrastructure/presentation` folders). That design was implemented and fully reviewed, then **replaced** with a flat `Controller`/`Service`/`Module` layering per feature — see [`specs/constitution.md`](constitution.md) ("Architecture and Clean Code") for the rationale, and `backend/src/` for the real, current code. This document is kept as a historical record of the original plan; it does not describe the codebase as it exists today. Only two prescriptive mentions below (Task 6, Self-Review Notes) were corrected to point at the real architecture — the rest (File Structure, Task 1, Task 3, Global Constraints) is left as originally written.
...
- **Constitution compliance**: flat `Controller`/`Service`/`Module` layering realized concretely — each `Controller` does routing only, each `Service` owns both the business logic and the data it operates on (no separate repository layer), and domain errors are typed exceptions extending `HttpException` directly (`common/exceptions/app.exception.ts`), mapped to status codes exactly once in the global `HttpExceptionFilter`; the idempotency cache and `erp-mock`'s exemption from the NestJS mandate are both documented in the constitution itself, not assumed. Every file follows the kebab-case `<name>.<role>.ts` convention (`.controller.ts`, `.service.ts`, `.module.ts`, `.dto.ts`). DTO + global `ValidationPipe` (no manual body-validation in a controller), one module/one responsibility (with `ProductsService` owning both catalog and stock explicitly justified against the circular-import alternative), `*.spec.ts` unit tests for the two places with actual branching logic, `*.e2e-spec.ts` for every endpoint — each rule from the constitution's "Architecture and Clean Code" and "Testing" sections has a concrete step realizing it, not just a mention.
